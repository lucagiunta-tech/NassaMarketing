...
export function TeamPlannerNMS({projects}){
  ...
  const editorialItems = useMemo(() => projects.flatMap(proj=>
  ...
  // Aggregates memoized — recalculate only when tasks/editorialItems change
  const tasksByMemberDay = useMemo(() => {
    const map = {};
    tasks.forEach(t => {
      const key = t.memberId + ":" + t.dateISO;
      if (!map[key]) map[key] = [];
      map[key].push(t);
    });
    return map;
  ...
  const editorialByMemberDay = useMemo(() => {
    const map = {};
    editorialItems.forEach(ei => {
      (ei.membersAssigned || []).forEach(mId => {
        const key = mId + ":" + ei.dateISO;
        if (!map[key]) map[key] = [];
        map[key].push(ei);
      });
    });
  ...
  const oreWeek = useMemo(() => {
    const result = {};
    members.forEach(m => {
      const manual = tasks.filter(t=>t.memberId===m.id).reduce((s,t)=>s+(t.ore||0),0);
      const editorial = editorialItems.filter(ei=>ei.membersAssigned.includes(m.id)).length;
      result[m.id] = manual + editorial;
    });
    return result;
  }, [members, tasks, editorialItems]);

  const oreDay = useMemo(() => {
    const result = {};
    tasks.forEach(t => { result[t.dateISO] = (result[t.dateISO]||0) + (t.ore||0); });
    return result;
  }, [tasks]);

  const tasksFor   = (mId, dIso) => tasksByMemberDay[mId+":"+dIso]   || [];
  const editorialFor = (mId, dIso) => editorialByMemberDay[mId+":"+dIso] || [];
  const clientList=[...new Set(projects.map(p=>p.name).filter(Boolean))];
  const allEditorialThisWeek = useMemo(() => [...editorialItems].sort((a,b)=>a.dateISO.localeCompare(b.dateISO)), [editorialItems]);
      ...
      {viewMode==="week"&&(
        <div className="tp-grid-wrap">
          <table className="tp-table">
            <thead><tr><th className="tp-th-member">Membro</th>{DAYS.map(d=><th key={d.iso} className={`tp-th-day ${d.iso===todayISO?"tp-today-col":""}`}><div>{d.label}</div><div className="tp-day-date">{d.short}</div>{(oreDay[d.iso]||0)>0&&<div className="tp-day-ore">{oreDay[d.iso]||0}h</div>}</th>)}<th className="tp-th-tot">Sett.</th></tr></thead>
            <tbody>
              {members.map(m=>{
                const tot=oreWeek[m.id]||0,pct=Math.min(100,Math.round((tot/(m.ore||40))*100));
                return(<tr key={m.id}>
