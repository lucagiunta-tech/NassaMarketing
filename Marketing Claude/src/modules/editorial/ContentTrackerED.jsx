...
import { SemaforoModal } from "./SemaforoNassa";
...
export function ContentTrackerED({ project, onUpdate }) {
  ...
  const [semaforoItem, setSemaforoItem] = useState(null);
  ...
  function move(id, status) {
    // Gate: se avanza da produzione a semaforo, apri la checklist Semaforo Nassa
    if (status === "semaforo") {
      const item = items.find(i => i.id === id);
      setSemaforoItem(item || { id });
      return;
    }
    upEd(ed => ({ ...ed, contentItems: (ed.contentItems||[]).map(i => i.id===id ? {...i, status} : i) }));
  }

  function confirmSemaforo(id) {
    upEd(ed => ({ ...ed, contentItems: (ed.contentItems||[]).map(i => i.id===id ? {...i, status:"semaforo"} : i) }));
    setSemaforoItem(null);
  }
  ...
  return(
    <div className="kan-wrap">
      ...
      {semaforoItem && (
        <SemaforoModal
          post={semaforoItem}
          onConfirm={() => confirmSemaforo(semaforoItem.id)}
          onCancel={() => setSemaforoItem(null)}
        />
      )}
    </div>
  );
