  ...
  return (
    <div className="lo-home">
      ...
      <div style={{display:"flex",gap:8,margin:"10px 0",flexWrap:"wrap"}}>
        {[
          { icon:"📋", label:"Kanban Board",    sub:"Pipeline contenuti",   view:"kanban",     color:"#7C3AED" },
          { icon:"📤", label:"Publishing Hub",  sub:"Caption AI + pubblica", view:"publishing", color:"#006EFF" },
          { icon:"📅", label:"Calendario",      sub:"Vista mensile",         view:"contenuti",  color:"#00C853" },
          { icon:"💡", label:"Idee Board",      sub:"Brainstorming",         view:"idee",       color:"#FF6B00" },
        ].map(s => (
          <button key={s.view} onClick={() => onNavigate(s.view)}
            style={{flex:1,minWidth:130,display:"flex",alignItems:"center",gap:9,padding:"9px 12px",background:"var(--w)",
              border:"1px solid var(--border)",borderRadius:8,cursor:"pointer",textAlign:"left",transition:".12s"}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor=s.color;e.currentTarget.style.background=s.color+"0D";}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor="var(--border)";e.currentTarget.style.background="var(--w)";}}>
            <span style={{fontSize:18,flexShrink:0}}>{s.icon}</span>
            <div><div style={{fontSize:11,fontWeight:700,color:"var(--ink)"}}>{s.label}</div>
              <div style={{fontSize:10,color:"var(--ink3)"}}>{s.sub}</div></div>
          </button>
        ))}
      </div>
