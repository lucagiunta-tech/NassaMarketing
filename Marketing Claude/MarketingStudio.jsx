...
import { P_PDM, P_PDC } from "./prompts";
import {
  FeedPreviewGrid,
  ListaView,
  CalendarSimpleED,
...
function ContenutiView({ project, onUpdate, members, onEdit, onAddNew, globalMeta }) {
  ...
  const statsCounts = FEED_STATI.reduce((acc,s)=>{ acc[s]=feedItems.filter(f=>f.stato===s).length; return acc; },{});

  let filtered = feedItems;
  if(filterPiat!=="tutti")     filtered=filtered.filter(f=>(f.piattaforme||[]).includes(filterPiat));
  if(filterStato!=="tutti")    filtered=filtered.filter(f=>f.stato===filterStato);
  if(filterFunnel!=="tutti")   filtered=filtered.filter(f=>(f.funnel||"").toUpperCase()===filterFunnel);
  if(filterPilastro!=="tutti") filtered=filtered.filter(f=>f.pilastro===filterPilastro);
  if(search.trim())            filtered=filtered.filter(f=>(f.titolo||"").toLowerCase().includes(search.toLowerCase()));

  function upFeed(fn){ onUpdate({...project,ed:{...(project.ed||{}),...fn(project.ed||{})}}); }
  function saveItem(item){ upFeed(ed=>({...ed,feedItems:(ed.feedItems||[]).some(f=>f.id===item.id)?(ed.feedItems||[]).map(f=>f.id===item.id?item:f):[...(ed.feedItems||[]),item]})); }
...
// ── Post Grid Card ───────────────────────────────────────────────────────────
function PostGridCard({ item, members, isSelected, onClick, onEdit, onSave, onDelete }) {
...
function EditorialeMain({ project, onUpdate, globalMeta }) {
...
// ─── PROJECT VIEW ─────────────────────────────────────────────────────────────
function ProjectView({project, onUpdate, onBack, globalMeta, onPortal}){
  ...
  return(
    <div className="proj-view">
      ...
      {module==="ed"&&(
        <EditorialeMain project={project} onUpdate={onUpdate} globalMeta={globalMeta}/>
      )}
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        {view==="project"&&activeProj&&(
          <ProjectView project={activeProj} onUpdate={handleUpdate} onBack={handleBack} globalMeta={globalMeta} onPortal={()=>openPortal(activeProj.clientId)}/>
        )}
      </div>
    </div>
  );
